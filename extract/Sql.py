...
class Database:
    ...
    # def update_data(client, data=None, columns=None, table_name=None, key_column_df=None, key_column_bd=None):
    #     update_instance = Update(table_name_to_update, client_name)
    #     update_instance.update(data_to_update, columns_to_update, key_column_df='codigo', key_column_bd=key_column_to_update)
    ...
    def select_data(self, query, columns=None):
        if columns is None:
            columns = ["*"]

        return spark.read.jdbc(url=self.jdbcUrl, table=f"({query}) AS custom_query", properties=self.connectionProperties)
            
    
    def write_streaming_data(self, streaming_data, query_name, table_name):
        query = streaming_data \
            .writeStream \
            .outputMode("append") \
            .foreachBatch(self.batch_write(table_name)) \
            .queryName(query_name) \
            .start()

        query.awaitTermination()

    def batch_write(self, micro_batch_df, batch_id, table_name):
        # Your custom logic for writing each micro batch
        micro_batch_df.write \
            .format("jdbc") \
            .option("url", self.jdbcUrl) \
            .option("dbtable", table_name) \
            .option("user", self.connectionProperties["user"]) \
            .option("password", self.connectionProperties["password"]) \
            .option("driver", self.connectionProperties["driver"]) \
            .mode("append") \
            .save()
